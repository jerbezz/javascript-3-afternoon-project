...
//Code Here
let totalPopulation = populations.reduce(function(total, current, index, array){
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(item, index, array){
  if(array[index].CP > 200){
    return array[index]
  }
})
...
let bobsTotal = purchases.filter(function(item){
  if(item.owner === "Bob"){
    return item
  }
}).map(function(item){
